...
if (isset($_GET['id'])) {
  // if the id is passed we want to check if the id is in the catalog array.
  $id = $_GET['id'];
  if(isset($catalog[$id])) {
    $item = $catalog[$id];
    // var_dump($item);
...
if(!isset($item)) {
  ...
  // this will make sure that nothing else will happen while the redirect is happening.
  exit;
}
